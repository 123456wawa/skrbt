-- ==========================================
-- سكربت blokspin الذكي | نسخة المسار الآمن المخصص (بدون تيليبورت)
-- الرمز السري: WAFI
-- ==========================================
...
if CoreGui:FindFirstChild("BlokSpinCustomWaypointsGui") then
    CoreGui:FindFirstChild("BlokSpinCustomWaypointsGui"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "BlokSpinCustomWaypointsGui"
...
GroceryButton.Activated:Connect(function()
    ...
    if groceryEnabled then
        ...
        GroceryButton.Text = "🚶 مشي مخصص آمن..."
        ...
        task.spawn(function()
            while groceryEnabled do
                ...
                if myHumanoid and myHrp then
                    -- الإحداثيات النهائية للطاولة
                    local targetPos = Vector3.new(164.98, 254.74, 204.42)
                    
                    -- [ خريطة النقاط المخصصة لتجنب الجدار والالتفاف من بعيد ]
                    local customPath = {
                        Vector3.new(180.50, 254.74, 222.00), -- نقطة 1: تراجع للخلف للممر الواسع البعيد عن الزاوية
                        Vector3.new(154.00, 254.74, 222.00), -- نقطة 2: الالتفاف حول زاوية المبنى بمسافة أمان واسعة
                        Vector3.new(154.00, 254.74, 204.42), -- نقطة 3: التقدم المباشر نحو مدخل البقالة بشكل مستقيم
                        targetPos                             -- نقطة 4: موقع طاولة التوظيف بالملي
                    }
                    
                    -- تنفيذ المشي الفيزيائي الطبيعي على النقاط الآمنة
                    for _, point in pairs(customPath) do
                        if not groceryEnabled then break end
                        
                        myHumanoid:MoveTo(point)
                        pressGameSprint() -- تفعيل الركض
                        
                        -- انتظر لين يوصل للنقطة الحالية قبل الذهاب للبعدها
                        local timeout = 0
                        while (myHrp.Position - point).Magnitude > 3 and timeout < 40 do
                            task.wait(0.05)
                            timeout = timeout + 1
                        end
                    end
                                ...
                                if string.find(nameL, "apply") or string.find(textL, "apply") or 
                                   ...
                                   string.find(nameL, "توظيف") or string.find(textL, "توظيف") then
                                    if guiObject.Visible then
                                        singleTapGui(guiObject) -- ضغطة بشرية موحدة ونظيفة
                    ...
                    -- 2. حل احتياطي للأزرار الأرضية القريبة جداً من الطاولة
