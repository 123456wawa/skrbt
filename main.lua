-- ==========================================
-- سكربت blokspin المطور | نظام التشغيل والإطفاء (Toggle) والتوظيف المضمون
-- الرمز السري: WAFI
-- ==========================================
...
-- زر البقالة بنظام الـ Toggle الجديد (رمادي في البداية)
        ...
        task.spawn(function()
            while groceryEnabled do
                ...
                if humanoid and hrp then
                    ...
                    -- 1. إذا كان اللاعب بعيد، يخليه يمشي للبقالة
                    if (hrp.Position - targetPos).Magnitude > 6 then
                        humanoid:MoveTo(targetPos)
                        humanoid.MoveToFinished:Wait() -- انتظر لين يخلص مشي لـ هناك
                        task.wait(1.5) -- انتظر ثانية ونصف كاملة عشان السيرفر يقتنع بموقعك الجديد وتتخطى الحماية
                    end
                    
                    -- 2. البحث عن زر التوظيف القريب وضغطه بشكل مضمون
                    for _, v in pairs(workspace:GetDescendants()) do
                        if v:IsA("ProximityPrompt") then
                            ...
                            if promptParent and (promptParent:IsA("BasePart") or promptParent:IsA("Model")) then
                                ...
                                -- التأكد أن هذا الزر داخل البقالة (قريب من الإحداثيات)
                                if (pPos - targetPos).Magnitude < 15 then
                                    -- تثبيت إحداثيات اللاعب فوق الزر بالضبط لضمان قبول السيرفر
                                    hrp.CFrame = CFrame.new(pPos) * CFrame.new(0, 0.5, 0)
                                    task.wait(0.5)
                                    
                                    -- محاكاة الضغط التلقائي
                ...
                task.wait(1) -- فحص وتكرار العملية كل ثانية طالما الزر أخضر (Autofarm مستمر)
            end
        end)
        
    else
        -- [الحالة: مطفأ -> يقلب رمادي]
