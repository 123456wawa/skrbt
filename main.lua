-- ==========================================
-- سكربت blokspin الذكي | نظام تفادي الجدران الآلي وضغطة التوظيف الموحدة
-- الرمز السري: WAFI
-- ==========================================
...
local PathfindingService = game:GetService("PathfindingService") -- نظام الحساب الذكي للطريق
local VirtualInputManager = game:GetService("VirtualInputManager")
local LocalPlayer = Players.LocalPlayer

if CoreGui:FindFirstChild("BlokSpinSmartGui") then
    CoreGui:FindFirstChild("BlokSpinSmartGui"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "BlokSpinSmartGui"
...
-- دالة كبس زر ركض اللعبة تلقائياً أثناء المشي البشري
...
-- دالة المحاكاة البشرية لإرسال ضغطة واحدة نظيفة لمنتصف الزر
        ...
        task.spawn(function()
            while groceryEnabled do
                ...
                if myHumanoid and myHrp then
                    ...
                    -- [ مرحلة حساب الطريق الذكي لتفادي الجدران بالكامل ]
                    ...
                    if path.Status == Enum.PathStatus.Success then
                        -- يتبع النقاط الأرضية بذكاء ويلف حول كل الجدران والعوائق
                        for _, waypoint in pairs(waypoints) do
                            if not groceryEnabled then break end
                            
                            -- إذا واجه عائق يتطلب القفز يطمر تلقائياً
                            if waypoint.Action == Enum.PathAction.Jump then
                                myHumanoid.Jump = true
                            end
                            
                            -- مشي فيزيائي طبيعي سريع
                            myHumanoid:MoveTo(waypoint.Position)
                            
                            -- تشغيل زر الركض الخاص باللعبة لزيادة السرعة
                            pressGameSprint()
                            
                            -- انتظر لين يوصل للنقطة الحالية عشان يروح للي بعدها
                            local timeout = 0
                            while (myHrp.Position - waypoint.Position).Magnitude > 3 and timeout < 10 do
                                task.wait(0.02)
                                timeout = timeout + 1
                            end
                        end
                    else
                        -- حل احتياطي ذكي لو فشل نظام الحساب (يمشي مباشرة)
                        myHumanoid:MoveTo(targetPos)
                    end
                    
                    if not groceryEnabled then break end
                    
                    -- وصلنا للبقالة بالملي وقف الحركة تماماً وثبت
                    myHumanoid:MoveTo(myHrp.Position)
                    GroceryButton.Text = "⏳ مزامنة التوظيف..."
                    task.wait(1.0) -- انتظر ثانية عشان السيرفر يسجل وقوفك الحقيقي على الأرض
                    
                    if not groceryEnabled then break end
                    
                    GroceryButton.Text = "⚡ ضغطة واحدة موحدة..."
                    local clickedSuccessfully = false
                    
                    -- 1. فحص وضغط زر الشاشة الـ Apply ضغطة واحدة نظيفة ومحاكية للبشر
                    local pGui = LocalPlayer:FindFirstChild("PlayerGui")
                    if pGui then
                                ...
                                if string.find(nameL, "apply") or string.find(textL, "apply") or 
                                   ...
                                   string.find(nameL, "توظيف") or string.find(textL, "توظيف") then
                                    if guiObject.Visible then
                                        singleTapGui(guiObject) -- ضغطة واحدة نظيفة ومثالية
                    ...
                    -- 2. إذا لم يجد زر في الشاشة، يضغط أزرار الماب الأرضية ضغطة واحدة موحدة
