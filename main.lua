-- ==========================================
-- سكربت blokspin الذكي | نسخة الترتيب الصحيح (المشي أولاً ثم التوظيف)
-- الرمز السري: WAFI
-- ==========================================
...
if CoreGui:FindFirstChild("BlokSpinLogicGui") then
    CoreGui:FindFirstChild("BlokSpinLogicGui"):Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "BlokSpinLogicGui"
...
-- تشغيل ركض اللعبة تلقائياً أثناء الرحلة
...
GroceryButton.Activated:Connect(function()
    ...
    if groceryEnabled then
        ...
        GroceryButton.Text = "🏃 جاري الذهاب للبقالة..."
        ...
        task.spawn(function()
            while groceryEnabled do
                ...
                if myHumanoid and myHrp then
                    ...
                    -- [ المرحلة الأولى: التركيز الكامل على المشي والركض وتفادي الجدران ]
                    ...
                    if path.Status == Enum.PathStatus.Success then
                        ...
                        -- حل احتياطي لو فشل حساب الطريق
                        myHumanoid:MoveTo(targetPos)
                    end
                    
                    if not groceryEnabled then break end
                    
                    -- [ المرحلة الثانية: التأكد الصارم من الوصول الفعلي قبل لمس أي زر ]
                    local distanceToTarget = (myHrp.Position - targetPos).Magnitude
                    if distanceToTarget > 5 then
                        -- لو لسه ما وصل، يرجع يعيد المحاولة ويمشي ولا يسوي شيء ثاني
                        GroceryButton.Text = "🔄 جاري تعديل المسار للبقالة..."
                        task.wait(0.5)
                        continue
                    end
                    
                    -- وصلنا بالملي! وقف وثبت حركتك
                    myHumanoid:MoveTo(myHrp.Position)
                    GroceryButton.Text = "⏳ وصلنا! جاري التوظيف الآن..."
                    task.wait(1.2) -- ثانية موازنة لضمان تسجيل السيرفر
                    
                    if not groceryEnabled then break end
                    
                    local clickedSuccessfully = false
                    
                    -- 1. فحص وضغط زر شاشة الـ Apply الخاص بالبقالة غصب
                    local pGui = LocalPlayer:FindFirstChild("PlayerGui")
                    if pGui then
                                ...
                                if string.find(nameL, "apply") or string.find(textL, "apply") or 
                                   ...
                                   string.find(nameL, "توظيف") or string.find(textL, "توظيف") then
                                    if guiObject.Visible then
                                        singleTapGui(guiObject) -- ضغطة بشرية نظيفة
                    ...
                    if clickedSuccessfully then
                        GroceryButton.Text = "✅ تم التوظيف في البقالة!"
                    else
                        GroceryButton.Text = "❌ لم يتم العثور على زر البقالة"
                    end
